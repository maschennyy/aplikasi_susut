...
class MeterDataRouteTest(unittest.TestCase):
    ...
    def test_pagination_parameters_are_validated_and_capped(self):
        invalid = self.client.get("/api/meter-data?page_size=0")
        self.assertEqual(invalid.status_code, 400)
        self.assertEqual(invalid.get_json(), {"error": "Page size harus berupa angka positif."})

        response = self.client.get("/api/meter-data?page=1&page_size=999")
